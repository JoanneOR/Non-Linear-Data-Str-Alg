...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	public int my_count_at_level(int level){
		//TO-DO
	{
		int count = 0;//used to keep track of the number of nodes found at the level.
		
		//checks if the current binary tree is empty. 
		//If it is empty, then it means there are no nodes in the tree and returns 0
		if(this.my_is_empty()) {
		return 0;
		}
		//It checks if the level is equal to 1. If it is then it returns 1 because there is only one node at the root level
		if(level == 1) {
		return 1;
		}
			
		//If the level is greater than 1 then it calls the function recursively on the left and right subtrees, subtracting 1 from the level each time to count the number of nodes at the next level. 
		//The count of nodes from the left and right subtrees is then added together
		return count += this.my_left_tree().my_count_at_level(level - 1) + this.my_right_tree().my_count_at_level(level - 1); 
	}
}
	...
	public boolean my_is_balanced(){
		...
		//TO-DO	
		if(this.my_is_empty()){//checks if the binary search tree is empty
		return true; 
		}

		//calculating the absolute difference between the heights of the left and right subtrees of the node
		//It uses the my_length method to get the height of each subtree. Getting the absolute value of the difference, 
		//gives the amount by which the left subtrees height differs from the right subtrees height disregarding which subtree is taller.
		int subtreeHeight = Math.abs(this.my_left_tree().my_length() - this.my_right_tree().my_length());
					
		if(subtreeHeight <= 1) //the difference in height between the left and right subtrees is at least 1 then the tree is balanced.
		//then my_is_balanced() is recursively called on the left and right subtrees and returns true only if both subtrees are balanced
		return this.my_left_tree().my_is_balanced() && this.my_right_tree().my_is_balanced();
		

		//if subtreeHeight is greater than 1 the tree is not balanced and returns false
		return false; 
	}
	...
	public int my_count_smaller_nodes(T1 key){
		...
		if(this.my_is_empty() == true){//checks if the binary search tree is empty
		return 0;
		}
		else
		{
			T1 rootValue = this.my_root().getKey(); //gets the key value of the root node of the binary search tree
			
			if(rootValue.compareTo(key) < 0 )//checks if the key value of the root node is less than the input key, using the compareTo() method
			 	
			   // the method adds 1 to the count to include the root node in the total count, and recursively calls the my_count_smaller_nodes() method on both the left and right subtrees
				return count += 1 + this.my_left_tree().my_count_smaller_nodes(key) + this.my_right_tree().my_count_smaller_nodes(key); 
		
			// counts the number of nodes with keys smaller than the input key in the left and right subtrees of the current node, excluding the current node
		return count = this.my_left_tree().my_count_smaller_nodes(key) + this.my_right_tree().my_count_smaller_nodes(key);  
		}
	}	
	...
	public int my_find_node_at_level(T1 key){
		...
		//TO-DO
		int count = 0; //used to keep track of the level of the node with the given key value
		
		if(this.my_is_empty()){
			return -1; //  If the tree is empty, the method returns -1 meaning that the node is not found. 
		}
		T1 rootKey = this.my_root().getKey();//gets the key value of the root node 
		
		
		if(key.compareTo(rootKey) < 0)//checks if the key value is less than the key value of the root node
		    //If it is, then the node with the given key value must be located in the left subtree of the root node, so the my_find_node_at_level() 
			//method is called recursively on the left subtree to search for the node.
			count = this.my_left_tree().my_find_node_at_level(key);
		
		else if(key.compareTo(rootKey) > 0)
			count = this.my_right_tree().my_find_node_at_level(key);	
		

		if(count == -1) {// when the node with the given key value is not found return -1
			return -1;
		}

		
		return count + 1; // returns the level at which a node with a given key value is found 
		                       //The +1 is added because the levels in the tree are counted starting from 0, but want to return the level starting from 1. 
